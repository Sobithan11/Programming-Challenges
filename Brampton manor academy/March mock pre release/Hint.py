...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer,Solution):
  """
  ...
  if PuzzleGrid[0][0] != 'X':
    ...
    print("Enter row column digit or H: ")
    print("(Press Enter to stop)")
    CellInfo = input()
    while CellInfo != EMPTY_STRING:
      InputError = False
      if len(CellInfo) != 3 and CellInfo!="H":
        InputError = True
      elif CellInfo=="H":
        ask=input("Enter row and column")
        row=int(ask[0])
        column=int(ask[1])
        Digit=Solution[row][column]
        Row=row
        Column=column
        PuzzleGrid[row][column]=Solution[row][column]
      else:
        Digit = CellInfo[2]
        try:
          Row = int(CellInfo[0])
  ...
  while not Finished:
    ...
    if MenuOption == 'L':
      ...
      if PuzzleGrid[0][0] != 'X':
        ...
        PuzzleGrid, Answer = SolvePuzzle(PuzzleGrid, Puzzle, Answer,Solution)
    elif MenuOption == 'X':
      Finished = True
    else:
